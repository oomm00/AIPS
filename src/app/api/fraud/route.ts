...
/**
 * /api/fraud  –  Standalone Fraud Engine endpoint
 *
...
import { FraudEngineInputSchema } from '@/lib/engineValidators';
...
import type { FraudOutput } from '@/lib/engineTypes';
  ...
  try {
    ...
    if (!simId) {
      ...
      simId = await createSimulationEvent({
        worker_id: workerId,
        zone_id: zoneId,
        event_type: 'fraud_standalone',
        payload_json: { standalone: true },
      });
    }

    const input = FraudEngineInputSchema.parse(rawBody);
    const result: FraudOutput = runFraudEngine(input);

    const runId = await persistFraudRun(simId, result);
    const auditId = await writeAuditLog({
      simulation_id: simId,
      engine_name: 'fraud',
      decision: result.status,
      confidence: result.work_probability,
